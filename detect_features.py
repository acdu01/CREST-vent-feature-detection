...
if __name__ == "__main__":
    INPUT_ROOT = "1_video_processing/output_img"
    WEIGHTS_PATH = "2_sp_implementation/saved_models/sp_v6"
    OUTPUT_ROOT = "2_sp_implementation/output"

    all_keypoints = []

    # Loop through each subfolder (each video)
    for video_name in os.listdir(INPUT_ROOT):
        ...
        # Call your existing function AS-IS
        ...
        # Collect stats from this video
        all_keypoints.extend(summary["num_keypoints"])

    # Global stats
    print("\n=========== Global Processing Summary ===========")
    print(f"Total images processed: {len(all_keypoints)}")
    print(f"Average keypoints per image: {sum(all_keypoints)/len(all_keypoints):.1f}")
